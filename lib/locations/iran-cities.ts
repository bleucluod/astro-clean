...
export function findIranCityByName(value: string) {
  ...
  return IRAN_CITY_OPTIONS.find((city) => {
    return (
      ...
      normalizeCitySearch(getIranCityDisplayName(city)) === normalizedValue ||
      city.id === normalizedValue ||
      String(city.sourceId) === normalizedValue
    );
  });
}
