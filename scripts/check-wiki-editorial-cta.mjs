...
const required = [
  ...
  ["components/AppShell.tsx", ["HALLEUS_WIKI_CTA_RUNTIME_V18", "HALLEUS_WIKI_CTA_SHORT_VIEWPORT_V22", "void card.offsetHeight;", 'data-wiki-chrome-control="back-to-top"', 'const revealProgress = 0.35;', 'const visibleDurationMs = 12000;', 'const exitDurationMs = 300;', 'new IntersectionObserver', "visibilitychange", "data-wiki-card-active", "data-wiki-end-cta-in-view", "pointerdown", "focusin", "prefers-reduced-motion: reduce"]],
  ["components/app-shell.module.css", ["HALLEUS_WIKI_CTA_CHROME_COLLISION_V12", "HALLEUS_WIKI_CTA_CHROME_CHOREOGRAPHY_V21", "body[data-wiki-card-active=\"true\"]", "body[data-wiki-end-cta-in-view=\"true\"]", "visibility: hidden !important;", "pointer-events: none !important;"]],
...
if (appShell.includes("visibleAttentionMs")) throw new Error("Reviewed CTA must not retain the visible-attention gate");
if (appShell.includes("minSafeViewportHeight")) throw new Error("Reviewed CTA must not suppress or pause on short viewport height");
if (!appShell.includes("__HALLEUS_WIKI_CTA_RUNTIME_V22__")) throw new Error("Reviewed CTA must use the V22 runtime key");
...
for (const legacyColor of ["#111923", "#35516a", "#b9e5ff", "#a9dfff", "#092235"]) if (css.includes(legacyColor)) throw new Error(`Wiki CTA retains legacy blue palette: ${legacyColor}`);
for (const token of [".editorialMobileCta[hidden]", "display: none !important;", "HALLEUS_WIKI_EDITORIAL_MOBILE_CTA_SHORT_VIEWPORT_V22", "background: #11141a;", "color: #f4f6f8;", "border: 1px solid rgba(255, 255, 255, 0.14);", "border-radius: 16px;", "box-shadow: 0 12px 32px rgba(0, 0, 0, 0.28);", "width: min(352px, calc(100vw - 28px));", "max-height: min(188px, 34dvh);", "color: #d6dde6;", "background: #e8ecf0;", "color: #11141a !important;", "min-height: 48px;", "width: 40px;", "height: 40px;", "@media (max-width: 720px) and (max-height: 520px)", "max-height: min(164px, calc(100dvh - 24px - env(safe-area-inset-bottom)));", "background: conic-gradient(", "#d8dde5 92% 97%", "mask-composite: exclude;", ".editorialMobileCta[data-state=\"visible\"]", ".editorialMobileCta[data-tab-visible=\"false\"]"]) {
