...
## v0.1.287 cross-section consistency reality

- The synthesis planner now exposes one ordered role list and anchors the primary house to the first selected role that lands in an active house.
- The main daily-life narrative follows only the challenge, support, and daily-bridge roles selected in the opening; it no longer repeats five angle/orb paragraphs.
- Exact angle, separation, and orb values remain available in the complete technical aspect inventory/table.
- Opening and closing practices are generated by one shared practice builder; the first weekly practice is reused verbatim in the final summary.
- The closing explicitly states that it preserves the opening thread and uses the evidence-backed primary house.
- Sample QA enforces eight sections, cross-section role continuity, shared practice wording, no technical angle detail in the main daily-life chapter, and a 1,450-word ceiling.
- No astrology calculation, aspect/orb policy, Placidus, Node/Lilith, or transit logic changed.
