...
## v0.1.265b live route guard before report engine claims

Workflow rule reinforced: report-engine/report-narrative claims must be checked against the live `/reports/[reportId]` path. ReportCard-only changes are not sufficient for report-detail product progress unless the route explicitly renders them or a guard proves the bridge.
